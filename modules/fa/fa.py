...
def goplus_fa(network, token):
    ...
    if score >= 20:
        is_passed = 1
        logger.info("Go is PASSED")
    elif score <= 20:
        is_passed = 0
    logger.info("Go score: ", score)
    return is_passed
...
def dexscreener_fa(network, pool):
    ...
    if bull_ratings >= (bear_ratings * 1.5):
        is_passed = 1
        logger.info("Dexscreener is PASSED")
...
def full_fa(name, token, lp, chain_short, chain_extra, chain):
    logger.debug(name, token, lp, chain_short, chain_extra, chain)
    ...
    logger.info("Score is: ", full_result)
    return full_result
