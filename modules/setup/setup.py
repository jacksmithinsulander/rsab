...
default.logFile = "rsab.log"
default.logSizeLimit = "500 MB"


def profit_taking_config():
    print("=======================================================================")
    profit_taking = input(
        'How many % profits do you want to take on each target levels? Answer with numbers ended by % sign, i.e. "50 %" or "100 %"(if you want to sell it all on first target)\n')
...
def bag_sizing():
    print("=======================================================================")
    dynamic_or_static = input(
        "For bagsizing, would you like each bag to be (A) a dynamic %age of the ammount of dry powder your wallet holds or (B) a static number? Examples: Every time buy for 1 % of the dry powder or everytime buy for 1 dollar no matter what\n")
    sizing = Object()
    if dynamic_or_static == "A":
        dynamic_size = input(
            "You chose dynamic bag sizing. How many percentages should each buy be?\n")
        sizing.type = "dynamic"
        sizing.universalSize = int(dynamic_size.removesuffix(" %"))
    elif dynamic_or_static == "B":
        static_size = input(
            "You chose static bag sizing. How many biden bucks would you like to gamble for each trade?\n")
        sizing.type = "static"
...
def fa_on():
    print("=======================================================================")
    fa_toggle = input(
        "FA (A) on or (B) off? (NOT RECOMENDED TO TURN OFF SINCE ITS ONLY CHECKING WETHER OR NOT YOU CAN ACTUALLY SELL THE SHITCOINS)\n")
    if fa_toggle == "A":
...
def fa_passes_settings():
    print("=======================================================================")
    fa_passes = input(
        "The standard amount of passes for the 3 FA parameters are 2/3. Would you like to make it (A) 1/3, (B) 3/3 or (C) leave it as is?\n")
    if fa_passes == "A":
        print("1/3")
        return 1
    elif fa_passes == "B":
...
def set_static_stoploss():
    stoploss = input(
        "How many % loss would you like your stoploss to be at?\n")
    stoploss_obj = Object()
...
def set_timebased_stoploss():
    deadline = input(
        "How many days would you like to hold the shitcoin for?\n")
    deadline_obj = Object()
...
def sl_chooser():
    ...
    sl_param_choose = input(
        "(A) 200MA, (B) static stoploss or (C) timebased stoploss\n")
...
def stoploss_management():
    print("=======================================================================")
    stoploss_settings = input(
        "The alternatives for potential stoplosses are (A) stoploss on the 200MA on the 4hr chart, (B) a static stoploss, (C) a timebased stoploss, (D) all of them, (E) some of them or (F) none of them. \n")
...
def spec_params():
    ...
    choosen_params = input(
        "Profit taking size (1), profit targets (2), purchasing asset rebalancing (3), bag sizing (4), FA on off toggle (5), FA strictness (6), stoploss management (7)\n")
...
def custom_choose():
    ...
    change_selection = input(
        "Do you want to change all of these parameters (A) or only some (B) of them?\n")
...
def param_choose_results():
    ...
    param_choose = input(
        "Do you want the standard (1) or custom (2) settings?\n")
    if param_choose == "1":
...
def chain_chooser():
    ...
    chains = input(
        "Ethereum mainnet (E), Arbitrum (A), Optimism (O), Polygon (P), Avalanche (V), Fantom (F), Binance smart chain (B) or ALL (ALL)\n")
